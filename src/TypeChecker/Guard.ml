...
let rec traverse
  ...
  match t with
  | App (t1, t2) as app ->
      begin match Whnf.to_whnf app env with
  ...
  | Lambda (_, _, _, body)          ->  traverse fn_var arg_var arg_pos ctx env body
  | Product (_, _, tp, body)        ->  traverse fn_var arg_var arg_pos ctx env tp;
                                        traverse fn_var arg_var arg_pos ctx env body
  | TypeArrow (tp1, tp2)            ->  traverse fn_var arg_var arg_pos ctx env tp1;
                                        traverse fn_var arg_var arg_pos ctx env tp2

  | Let (_, _, t1, tp, t2)          ->  traverse fn_var arg_var arg_pos ctx env t1;
  ...
  | IfExpr (c, t1, t2)              ->  traverse fn_var arg_var arg_pos ctx env c;
                                        traverse fn_var arg_var arg_pos ctx env t1;
                                        traverse fn_var arg_var arg_pos ctx env t2
  | EqType (t1, t2, tp)             ->  traverse fn_var arg_var arg_pos ctx env t1;
                                        traverse fn_var arg_var arg_pos ctx env t2;
                                        traverse fn_var arg_var arg_pos ctx env tp
  | Refl (t1, tp)                   ->  traverse fn_var arg_var arg_pos ctx env t1;
                                        traverse fn_var arg_var arg_pos ctx env tp
  | Subst (_, _, t1, t2, t3)        ->  traverse fn_var arg_var arg_pos ctx env t1;
                                        traverse fn_var arg_var arg_pos ctx env t2;
                                        traverse fn_var arg_var arg_pos ctx env t3
  | Hole _ | Type | Kind
  | IntType | StringType | BoolType
  | IntLit _ | StringLit _ | BoolLit _ | Var _ -> ()
