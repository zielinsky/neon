...
let create_infer_type_error (pos : ParserAst.position) (error_msg: string) (term: ParserAst.uTerm) (env: env) : string =
  "Error at " ^ (Int.to_string pos.start.pos_lnum) ^ ":" ^ (Int.to_string pos.start.pos_bol) ^ "\n" ^
...
let rec infer_type ((_, termEnv) as env : env) ({pos; data = t}: ParserAst.uTerm) : (term * term) = 
  ...
  | Var x -> 
    ...
    | None -> failwith (create_type_checker_error pos ("Variable " ^ x ^ " not found"))
    end
  | Lambda (_, None, _) -> failwith (create_type_checker_error pos "Can't infer the type of lambda with omitted argument type")
  | Lambda (x, Some tp, t) -> 
    ...
    begin match tp_of_tp with
      ...
      | _ -> failwith (create_type_checker_error pos "The type of Lambda argument type must be either Type or Kind")
    end
  | Product (x, tp, t) ->
    let (tp, tp_of_tp) = infer_type env tp in
    begin match tp_of_tp with
      | Type | Kind | Triangle -> 
        ...
        | _ -> failwith (create_type_checker_error pos "The type of Product body type must be either Type or Kind")
        end
      | _ -> failwith (create_type_checker_error pos "The type of Product argument type must be either Type or Kind")
    end
  | TypeArrow (tp1, tp2) ->
    ...
    begin match tp_of_tp1 with
      | Type | Kind -> 
        ...
        | _ -> failwith (create_type_checker_error pos "The type of Type Arrow body type must be either Type or Kind")
        end
      | _ -> failwith (create_type_checker_error pos "The type of Type Arrow argument type must be either Type or Kind")
    end
  | App (t1, t2) ->
    ...
    | _ -> failwith (create_type_checker_error pos "The type of Application's first argument must be a Product")
    end
  | TermWithTypeAnno (t, tp) -> 
    let (tp, tp_of_tp) = infer_type env tp in 
    begin match tp_of_tp with
    | Type | Kind -> (check_type env t tp, tp)
    | _ -> failwith (create_type_checker_error pos "Type annotation must be a Type or Kind")
    end
  | Let (x, t1, t2) -> 
  ...
  | LemmaDef(x, t1) -> 
    ...
    t1, tp_t1
  | Hole x -> failwith (create_type_checker_error pos "Trying to infer the type of a Hole " ^ x)
...
and check_type ((_, termEnv) as env : env) ({pos; data = t} as term: ParserAst.uTerm) (tp: term) : term =
  ...
  | Lambda (x, None, body) -> 
    ...
    | _ -> failwith (create_type_checker_error pos "The type of Lambda must be a Product")
    end
  | Lambda (x, Some x_tp, body) ->
    ...
    | _ -> failwith (create_type_checker_error pos "Lambda must be lambda lolz")
    end
  | Kind -> failwith (create_type_checker_error pos "Kind doesn't have a type")
  | Let (x, t1, t2) -> 
    let (t1, tp_t1) = infer_type env t1 in
    let fresh_var = add_to_env env x (Transparent (t1, tp_t1)) in
