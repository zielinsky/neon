...
let create_infer_type_error (pos : ParserAst.position) (error_msg: string) (term: ParserAst.uTerm) (env: env) : 'a =
  let _ = print_endline ("While infering the type of term: " ^ uterm_to_string(term) ^
   "\n\n The following error accured:\n\t" ^ error_msg ^ "\n"
   ^ "\nThe state of the environment at that moment:\n" ^ env_to_string env) in
   failwith ("Error at line " ^ (Int.to_string pos.start.pos_lnum) ^ ":" ^ (Int.to_string (pos.start.pos_cnum - pos.start.pos_bol)))

let create_check_type_error (pos : ParserAst.position) (error_msg: string) (term: ParserAst.uTerm) (tp: tp) (env: env) : 'a =
let _ = print_endline ("While checking the type of term: " ^ uterm_to_string(term) ^ "\nwith expected type: " ^
 term_to_string tp ^"\n\nThe following error accured:\n\t" ^ error_msg ^ "\n"
  ^ "\nThe state of the environment at that moment:\n" ^ env_to_string env) in
  failwith ("Error at line " ^ (Int.to_string pos.start.pos_lnum) ^ ":" ^ (Int.to_string (pos.start.pos_cnum - pos.start.pos_bol)))
...
let rec infer_type ((_, termEnv) as env : env) ({pos; data = t} as term: ParserAst.uTerm) : (term * term) = 
  ...
  | Lambda (_, None, _) ->  (create_infer_type_error pos "Can't infer the type of lambda with omitted argument type" term env)
  | Lambda (x, Some tp, t) -> 
    ...
    begin match tp_of_tp with
      ...
      | _ ->  (create_infer_type_error pos "The type of Lambda argument type must be either Type or Kind" term env)
    end
  | Product (x, tp, t) ->
    let (tp, tp_of_tp) = infer_type env tp in
    begin match tp_of_tp with
      | Type | Kind | Triangle -> 
        ...
        | _ ->  (create_infer_type_error pos "The type of Product body type must be either Type or Kind" term env)
        end
      | _ ->  (create_infer_type_error pos "The type of Product argument type must be either Type or Kind" term env)
    end
  | TypeArrow (tp1, tp2) ->
    ...
    begin match tp_of_tp1 with
      | Type | Kind -> 
        ...
        | _ ->  (create_infer_type_error pos "The type of Type Arrow body type must be either Type or Kind" term env)
        end
      | _ ->  (create_infer_type_error pos "The type of Type Arrow argument type must be either Type or Kind" term env)
    end
  | App (t1, t2) ->
    ...
    | _ ->  (create_infer_type_error pos "The type of Application's first argument must be a Product" term env)
    end
  | TermWithTypeAnno (t, tp) -> 
    let (tp, tp_of_tp) = infer_type env tp in 
    begin match tp_of_tp with
    | Type | Kind -> (check_type env t tp, tp)
    | _ ->  (create_infer_type_error pos "Type annotation must be a Type or Kind" term env)
    end
  | Let (x, t1, t2) -> 
  ...
  | LemmaDef(x, t1) -> 
    ...
    t1, tp_t1
  | Hole x ->  (create_infer_type_error pos ("Trying to infer the type of a Hole " ^ x) term env)
...
and check_type ((_, termEnv) as env : env) ({pos; data = t} as term: ParserAst.uTerm) (tp: term) : term =
  ...
  | Type | Var _ | App _ | Product _ | TermWithTypeAnno _ | TypeArrow _ -> 
    ...
    if equiv tp t_tp termEnv then t else  (create_check_type_error pos ("The expected type was: " ^ term_to_string t_tp) term tp env)
  | Lambda (x, None, body) -> 
    ...
    | _ ->  (create_check_type_error pos "The type of Lambda must be a Product" term tp env)
    end
  | Lambda (x, Some x_tp, body) ->
    ...
    | Lambda (_, _, arg_tp, _) as lambda -> 
      ...
      if equiv x_tp arg_tp termEnv then lambda else 
        (create_check_type_error pos
       ("Got: " ^ term_to_string x_tp ^ "\nThe expected type of lambda argument was: " ^ term_to_string arg_tp) term tp env)
    | _ ->  (create_check_type_error pos "Lambda must be lambda lolz" term tp env)
    end
