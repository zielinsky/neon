...
let rec term_to_string (t : term) : string =
  ...
  let rec uterm_to_string ({pos; data = t}: uTerm) : string =
    ...
    | Var nm -> nm
    | Lambda (nm, tp_x, body) -> 
      begin match tp_x with
      | Some tp_x -> "(λ" ^ nm ^ ":" ^ (uterm_to_string tp_x) ^ " => " ^ (uterm_to_string body) ^ ")"
      | None -> "(λ" ^ nm ^ " => " ^ (uterm_to_string body) ^ ")"
      end
    | Product (nm, tp_x, body) -> "(Π" ^ nm ^ ":" ^ (uterm_to_string tp_x) ^ " => " ^ (uterm_to_string body) ^ ")"
    | App (t1, t2) -> "(" ^ (uterm_to_string t1) ^ ") (" ^ (uterm_to_string t2) ^ ")"
    | Let (nm, t1, t2) -> "(let " ^ nm ^ "=" ^ (uterm_to_string t1) ^ " in\n\t" ^ (uterm_to_string t2)
    | LetDef (nm, t1) -> "(let " ^ nm ^ "=" ^ (uterm_to_string t1)
    | Lemma (nm, t1, t2) -> "(lemma " ^ nm ^ "=" ^ (uterm_to_string t1) ^ " in\n\t" ^ (uterm_to_string t2)
    | LemmaDef (nm, t1) -> "(lemmaDef " ^ nm ^ "=" ^ (uterm_to_string t1)
    | Hole nm -> nm
    | TypeArrow (tp1, tp2) -> "(" ^ (uterm_to_string tp1) ^ " -> " ^ (uterm_to_string tp2) ^ ")"
    | TermWithTypeAnno (t1, t2) ->  "(" ^ (uterm_to_string t1) ^ " : " ^ (uterm_to_string t2) ^ ")" 
